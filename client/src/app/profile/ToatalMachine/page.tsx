...
function Page() {
  ...
  const hasMachines = userMachines && userMachines.length > 0 && userMachines.some(machine => machine.status === "active")
  const hasShares = userShares && userShares.shares && userShares.shares.length > 0
          ...
          <div className="min-h-screen space-y-8 bg-zinc-950 p-6">
            <div className="mx-auto max-w-7xl">
              ...
              {(hasMachines || machinesLoading) && (
                <div className="mb-12">
                  <h3 className="mb-4 text-2xl font-bold text-white">Your Mining Machines</h3>
              ...
              {(hasShares || sharesLoading) && (
                <div>
                  <h3 className="mb-4 text-2xl font-bold text-white">Your Mining Shares</h3>
                  <UserSharesDashboard />
                </div>
              )}
              
              {/* If no assets at all */}
              {!machinesLoading && !sharesLoading && !hasMachines && !hasShares && (
                <div className="flex min-h-[300px] items-center justify-center rounded-2xl border border-zinc-800 bg-black">
                  <div className="text-center">
                    ...
                    <p className="mt-2 text-zinc-500">
                      Visit the marketplace to purchase your first mining machine or shares.
                    </p>
                  </div>
                </div>
              )}
